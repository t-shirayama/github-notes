...
| コマンド | 取り込むもの |
| -------- | ------------ |
| `git merge main` | ローカルの `main` |
| `git merge origin/main` | 最後に `fetch` した時点の `origin/main` |
| `git fetch origin` + `git merge origin/main` | リモートの最新情報を取得してから、更新後の `origin/main` |

どのコマンドも、今いるブランチに取り込みます。実行前に `git status` や `git branch --show-current` で、取り込み先のブランチを確認します。
