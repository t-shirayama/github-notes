...
このように、mainなどのベースブランチの変更を作業ブランチへ取り込むことを、現場では「バックマージ」と呼ぶことがあります。Git公式コマンド名ではなく、実際の操作は `git merge main` です。
...
Windows標準のPowerShellでは `grep` や `xargs` が使えない場合があります。また、削除対象を必ず確認してから実行します。
